module PurolatorWebServices
  module Definitions
    ...
    MODULE_PATH = File.join(%w(purolator_web_services definitions))
    ...
    def load_definitions(lib_dir)
      $VERBOSE = $VERBOSE.tap do
        ...
        Dir.glob(File.join(lib_dir, MODULE_PATH, "*Driver.rb")).each do |definition_file|
          begin
            require File.join(MODULE_PATH, File.basename(definition_file))
          rescue Exception
            raise "Error loading Purolator definition file #{definition_file}: #{$!.message}"
          end
        end
        $:.shift
        $:.shift
    ...
    def generate_definitions(lib_dir, *wsdl_files)
      ...
      wsdl_files.each do |wsdl_file|
        ...
        worker.opt.update(
          "module_path"      => "PurolatorWebServices::Definitions",
